...
import { TableDataCell, TableDataCellProps } from './TableDataCell'
...
export interface TableProps extends Omit<TableHTMLAttributes<HTMLTableElement>, 'align'> {
  ...
  footer?: FooterItem[]
  /**
   * Enable a hover state on table rows within a `<TableBody>`.
   */
  hover?: boolean
  /**
  ...
  small?: boolean
  /**
   * Add zebra-striping to any table row within the `<TableBody>`.
   */
  striped?: boolean
  /**
   ...
   *
   * @link https://coreui.io/react/docs/components/table/#Tablefoot
   * @since 4.3.0
   */
  tableFootProps?: TableFootProps
  /**
   * Properties that will be passed to the table head component.
   *
   * @link https://coreui.io/react/docs/components/table/#Tablehead
   * @since 4.3.0
   */
    ...
    const Table = () => (
      <table className={_className} {...rest} ref={ref}>
        {((caption && caption !== 'top') || captionTop) && (
          <TableCaption>{caption || captionTop}</TableCaption>
        )}
        {columns && (
        ...
        {items && (
          <TableBody>
            {items.map((item: Item, index: number) => (
              <TableRow {...(item._props && { ...item._props })} key={index}>
          ...
          <TableFoot {...tableFootProps}>
            <TableRow>
              {footer.map((item: FooterItem, index: number) => (
                <TableDataCell {...(item._props && { ...item._props })} key={index}>
...
Table.propTypes = {
  ...
  tableFootProps: PropTypes.shape({ ...TableFoot.propTypes }),
  tableHeadProps: PropTypes.shape({ ...TableHead.propTypes }),
}
